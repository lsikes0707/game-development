...
function love.load()
	...
	-- set up our sounds effects; later, we can just index this
	...
	-- either going to be 1 or 2; whoever is scored on gets to
	-- serve the following turn
	servingPlayer = 1

	-- player who won the game; not set to a proper value until
	-- we reach that state in the game
	winningPlayer = 0

	-- the state of our game; can be any of the following:
	-- 1. 'start' (the beginning of the game, before first serve)
	-- 2. 'serve' (waiting on a key press to serve the ball)
	-- 3. 'play' (the ball is in play, bounding between paddles)
	-- 4. 'done' (the game is over, with a victor, ready for 
	-- restart
	gameState = 'start'
end
...
--[[
	love.resize(w, h)
	Called by LOVE whenever we resize the screen; here, we just
	want to pass in the width and height to push so our virtual
	resolution can be resized as needed.
]]
...
function love.update(dt)
	if gameState == 'serve' then
		-- before swithcing to play, initialize ball's velocity based
	...
	elseif gameState == 'play' then
		-- detect ball collision with paddles, reversing dx if
		-- true and slightly increasing it, then altering the dy
		-- based on the position of collision
		if ball:collides(player1) then
			ball.dx = -ball.dx * 1.03
			ball.x = player1.x + 5

			-- keep velocity going in the same direction, but
			-- randomize it
			if ball.dy < 0 then
		...
		-- account for the ball's size by using -4
		...
		if ball.x < 0 then
			...
			-- if we've reached a score of 10, the game is over; set
			-- the state to done so we can show the victory message
			if player2Score == 10 then
				winningPlayer = 2
				gameState = 'done'
			else
				gameState = 'serve'
				-- places the ball in the middle of the screen, no
				-- velocity
				ball:reset()
			end
		end
...
--[[
	function love.draw()
	Called af://github.com/games50/pong/tree/master/pong-8 ter update by LOVE2D, used to draw anything to the screen, updated or otherwise.
]]
function love.draw()
	...
	-- show the score before ball is rendered so it can move over
	-- the text
	displayScore()
